...
# Sends to RECIPIENT via reports@example.com. Skips send when 0 highs.
#
# Uses the Wazuh OpenSearch indexer directly.
#
# Failure mode (SOC 1.4):
#   Pre-send selftest at /home/user/bin/wazuh-indexer-selftest.sh verifies
...
#   `crab-meat-repos/agentic-ai/scripts/soc/test_wazuh_morning_highlights.py`
...
#         Bypass the /home/user/bin/wazuh-indexer-selftest.sh pre-send
...
#   Commits in `crab-meat-repos/agentic-ai` branch `feat/morning-
#   highlights-selftest` (commit 19adcb99) — the script-side changes
#   are host-side only; `/home/user/bin/` is not git-tracked.
...
LOGFILE="${LOGFILE:-/home/user/logs/wazuh-high.log}"
...
if [[ "${WAZUH_HIGH_SKIP_SELFTEST:-0}" != "1" ]]; then
  LOG_TAG="${LOG_TAG:-wazuh-morning}" /home/user/bin/wazuh-indexer-selftest.sh || {
...
HEARTBEAT="${HEARTBEAT:-/home/user/logs/wazuh-heartbeat.status}"
RED_FLAG="${RED_FLAG:-/home/user/logs/wazuh-red-alerts.flag}"
...
---
Generated by /home/user/bin/wazuh-morning-highlights.sh (cron 0 8 * * *).
Snapshot:  $SNAP
Heartbeat: $HEARTBEAT
EOF
)
